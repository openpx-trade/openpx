...
use px_crypto::CryptoPriceWebSocket as InnerCryptoPriceWebSocket;
...
fn parse_source(s: &str) -> Result<CryptoPriceSource> {
...
pub struct CryptoPriceWebSocket {
    inner: Arc<Mutex<InnerCryptoPriceWebSocket>>,
}
...
impl CryptoPriceWebSocket {
    ...
    pub fn new() -> Self {
        Self {
            inner: Arc::new(Mutex::new(InnerCryptoPriceWebSocket::new())),
        }
    }
    ...
    #[napi]
    pub async fn subscribe(&self, source: String, symbols: Vec<String>) -> Result<()> {
        let source = parse_source(&source)?;
        let ws = self.inner.clone();
        let rt = get_runtime();
        rt.spawn(async move { ws.lock().await.subscribe(source, &symbols).await })
            .await
            .map_err(to_napi_err)?
            .map_err(to_napi_err)
    }
    ...
    #[napi]
    pub async fn unsubscribe(&self, source: String, symbols: Vec<String>) -> Result<()> {
        let source = parse_source(&source)?;
        let ws = self.inner.clone();
        let rt = get_runtime();
        rt.spawn(async move { ws.lock().await.unsubscribe(source, &symbols).await })
            .await
            .map_err(to_napi_err)?
            .map_err(to_napi_err)
    }
    ...
    /// Subscribe to price updates via callback.
    /// The callback receives each CryptoPrice as a JSON object.
    #[napi]
    pub async fn on_price_update(
        &self,
        #[napi(ts_arg_type = "(err: Error | null, price: any) => void")]
