...
- **kalshi**: `KalshiConfig` gains a `subaccount: Option<u32>` field and `with_subaccount()` builder; `fetch_balance` and `fetch_balance_raw` now pass `?subaccount=N` for FCM subaccount-scoped balance queries. ([#47](https://github.com/openpx-trade/openpx/pull/47))
