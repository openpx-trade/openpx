...
- **kalshi**: `Exchange::fetch_user_activity()` is now implemented — returns the authenticated user's fill history (fees, timestamps, maker/taker) from `GET /portfolio/fills`. `ExchangeInfo::has_fetch_user_activity` is now `true` for Kalshi.
