...
| `market_id` | `string` | Yes | K P | Native market ID |
| `outcome` | `string \| null` | No | K P | Outcome that was traded (e.g., `"Yes"`, `"No"`) |
| `price` | `number` | Yes | K P | Trade price, normalized 0-1 |
| `side` | `string \| null` | No | K P | Trade side label |
| `size` | `number` | Yes | K P | Number of contracts traded |
| `source_channel` | `string` | Yes | K P | Data source identifier (e.g., `"websocket"`) |
| `timestamp` | `string \| null` | No | K P | Trade timestamp (ISO 8601) |
| `trade_id` | `string \| null` | No | K P | Exchange-assigned trade identifier |
...
| `has_fetch_events` | `boolean` | Yes | K P | Whether fetching event/group market listings is supported |
