...
| Field | Type | Required | Exchanges | Description |
|-------|------|----------|-----------|-------------|
| `created_at` | `string` | Yes | K P O | Fill execution timestamp (ISO 8601) |
| `fee` | `number` | Yes | K O | Fee charged for this fill in USD. Polymarket: always `0.0` via REST |
| `fill_id` | `string` | Yes | K P O | Unique fill identifier |
| `is_taker` | `boolean` | Yes | K | Whether this fill was on the taker side. Polymarket/Opinion: always `false` via REST |
| `market_id` | `string` | Yes | K P O | Native market ID the fill occurred on |
| `order_id` | `string` | Yes | K O | Parent order ID that generated this fill. Polymarket: empty string via REST |
| `outcome` | `string` | Yes | K P O | Outcome that was traded (e.g., `"Yes"`, `"No"`) |
| `price` | `number` | Yes | K P O | Execution price, normalized 0-1 |
| `side` | `OrderSide` | Yes | K P O | Whether the order was a `buy` or `sell` |
...
| Field | Type | Required | Exchanges | Description |
|-------|------|----------|-----------|-------------|
| `close` | `number` | Yes | K P O | Closing price for this period, normalized 0-1 |
| `high` | `number` | Yes | K P O | Highest price during this period, normalized 0-1 |
| `low` | `number` | Yes | K P O | Lowest price during this period, normalized 0-1 |
| `open` | `number` | Yes | K P O | Opening price for this period, normalized 0-1 |
| `open_interest` | `number \| null` | No | K | Open interest at this candle's close. Only available from Kalshi |
| `timestamp` | `string` | Yes | K P O | Period start timestamp (ISO 8601, UTC) |
| `volume` | `number` | Yes | K O | Trade volume in contracts for this period. Polymarket: always `0.0` |
...
- **`exchange_time`** — server-authoritative UTC timestamp. Always prefer this for trade sequencing and cross-exchange correlation.
- **`received_at`** — local UTC timestamp captured at socket read. Compare `received_at - exchange_time` for feed lag.
- **`seq`** — per-market monotonic sequence number. Use for gap detection.

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `seq` | `number` | Yes | Per-market monotonic sequence number for gap detection |
| `exchange_time` | `string \| null` | No | Server-authoritative event timestamp (ISO 8601, UTC) |
| `received_at` | `string` | Yes | Local capture timestamp (ISO 8601, UTC) |
...
# {
#     "seq": 1,
#     "exchange_time": "2025-03-14T12:00:00Z",  # or None
#     "received_at": "2025-03-14T12:00:00.123Z",
#     "data": { ... }  # OrderbookUpdate or ActivityEvent
# }
```
...
| Variant | Description |
|---------|-------------|
| **Snapshot** | Full orderbook state. Sent on first subscribe and after reconnection. Contains complete `bids` and `asks` arrays. Polymarket snapshots include a `hash` for integrity verification. |
...
| `has_fetch_fills` | `boolean` | Yes | K P O | Whether fill/trade history queries are supported |
...
| `has_fetch_orderbook_history` | `boolean` | Yes | P | Whether historical orderbook snapshots are available. Polymarket serves this via S3 Parquet pipeline, but `describe()` returns `false` because data is not proxied through CLOB |
| `has_fetch_positions` | `boolean` | Yes | K P O | Whether position queries are supported |
| `has_fetch_price_history` | `boolean` | Yes | K P O | Whether OHLCV candlestick data is available |
| `has_fetch_trades` | `boolean` | Yes | K P O | Whether public trade tape queries are supported |
