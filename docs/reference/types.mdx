...
| Field | Type | Description |
|---|---|---|
| `away_team` | `['string', 'null']` *(optional)* | Away team display name. Same caveat as `home_team`. |
| `home_team` | `['string', 'null']` *(optional)* | Home team display name. May be `None` on providers that don't model teams as first-class (Kalshi). |
| `id` | [`GameId`](#gameid) |  |
| `league` | `string` | Canonical league id (`League::id`). Always set. |
| `provider` | `string` | Provider id this game was fetched from (e.g., "espn", "kalshi", "polymarket"). |
| `raw_status` | `['string', 'null']` *(optional)* | Verbatim status string from the provider before normalization. Useful when `status` is `GameStatus::Unknown` (caller can branch on `raw_status`). |
| `start_time` | `['string', 'null']` *(optional)* | Scheduled start time. May be `None` for providers that publish games only after kickoff. |
...
| `date` | `['string', 'null']` *(optional)* | Restrict to games on a specific calendar day (UTC). |
| `league` | `['string', 'null']` *(optional)* | Restrict to one league id (e.g., "nfl"). |
| `limit` | `['integer', 'null']` *(optional)* | Page size hint; providers may clamp. |
| `status` | [`GameStatus`](#gamestatus) \| `null` *(optional)* | Restrict to a normalized status. |
| `team` | `['string', 'null']` *(optional)* | Restrict to games involving a team (matches home or away). |
...
| Field | Type | Description |
|---|---|---|
| `clock` | `['string', 'null']` *(optional)* | Provider-formatted clock (e.g., "12:34"). Remaining-vs-elapsed semantics are sport-specific; surfaces verbatim. |
| `ended` | `boolean` |  |
| `game_id` | [`GameId`](#gameid) |  |
| `live` | `boolean` |  |
| `period` | `['string', 'null']` *(optional)* | Provider-formatted period label (e.g., "Q2", "Halftime", "9th Inning"). Kept as a string because semantics differ per sport. |
| `raw_status` | `['string', 'null']` *(optional)* | Verbatim status string from the provider; useful when `status` is `Unknown`. |
| `score` | [`Score`](#score) \| `null` *(optional)* | Current score. `None` until the game starts. |
| `status` | [`GameStatus`](#gamestatus) |  |
| `updated_at` | `string` | Timestamp this state was observed (provider-supplied if available, else receive-time on the client). |
...
| Field | Type | Description |
|---|---|---|
| `abbreviation` | `['string', 'null']` *(optional)* | Common abbreviation (e.g., "NFL", "NBA"). Optional — not every league has one. |
| `id` | `string` | Canonical league id (lowercase, no spaces). Stable across providers. |
| `name` | `string` | Human-readable display name. |
| `sport_id` | `string` | Back-reference to the parent `Sport::id`. |
...
| Field | Type | Description |
|---|---|---|
| `away` | `['integer', 'null']` *(optional)* |  |
| `home` | `['integer', 'null']` *(optional)* |  |
| `raw` | `['string', 'null']` *(optional)* | Verbatim provider score string when home/away can't be normalized (e.g., "0-0|2-0|Bo3", "+5/-3"). |
...
| Field | Type | Description |
|---|---|---|
| `id` | `string` | Canonical sport id (lowercase, no spaces). Stable across providers. |
| `name` | `string` | Human-readable display name. |
