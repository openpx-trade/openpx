//! Real-time sports score streaming via the Polymarket Sports WebSocket
//! (`wss://sports-api.polymarket.com/ws`). No auth, no subscription
//! messages — connect and receive all active events. Uses TEXT-level
//! `"ping"`/`"pong"` keepalive (not WebSocket protocol pings).
//!
//! This is the internal driver behind Polymarket's `impl SportsProvider`
//! (see `sports.rs` in this crate). It was lifted verbatim from the
//! `px-sports` crate as part of the sports data category reorg.
