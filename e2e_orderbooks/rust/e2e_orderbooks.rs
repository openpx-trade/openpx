...
fn assert_book_well_formed(book: &Orderbook, asset_id: &str, label: &str) {
    ...
    if !book.asset_id.is_empty() {
        assert!(
            book.asset_id == asset_id || book.asset_id.contains(asset_id) || asset_id.contains(&book.asset_id),
...
fn assert_stats_consistent_with_book(stats: &OrderbookStats, book: &Orderbook, label: &str) {
    assert_eq!(stats.asset_id, book.asset_id, "{label}: stats.asset_id drift");
...
async fn unified_orderbook_shape_across_exchanges() {
    let Some(kalshi) = make_exchange("kalshi") else { return };
    let Some(polymarket) = make_exchange("polymarket") else { return };
